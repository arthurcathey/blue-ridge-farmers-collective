      ...
      if ($market) {
        ...
        $datesStmt = $db->prepare('
          SELECT 
            ...
            weather_status_mda,
            notes_mda
          FROM market_date_mda
          WHERE id_mkt_mda = :market_id
            AND date_mda >= CURDATE()
            AND status_mda != "cancelled"
          ORDER BY date_mda ASC, start_time_mda ASC
          LIMIT 10
        ');
        $datesStmt->execute([':market_id' => $market['id_mkt']]);
