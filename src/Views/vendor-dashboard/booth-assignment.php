  ...
  <div class="mt-6 grid grid-cols-1 gap-4 md:gap-6 lg:grid-cols-3">
    ...
    <div class="lg:col-span-2">
      <section class="card">
        ...
        <p class="text-muted mb-4 text-sm"><?= h($layout['name_mla']) ?> - <?= $layout['booth_count_mla'] ?> total booths</p>
        ...
        <div class="grid gap-2 rounded border border-gray-300 bg-gray-100 p-3 sm:gap-3 sm:p-4" style="grid-template-columns: repeat(auto-fill, minmax(80px, 1fr)); min-width: 0;">
          <?php foreach ($booths as $booth): ?>
            ...
            <div class="booth-display <?php
              ...
              <?php if ($isMyBooth): ?>
                <div class="mt-2 border-t border-yellow-300 pt-2 text-xs">
                  <div class="font-semibold text-yellow-700">✓ Your Booth</div>
                </div>
              <?php elseif (!$isAvailable): ?>
                <div class="mt-2 border-t border-gray-300 pt-2 text-xs text-gray-600">Assigned</div>
              <?php else: ?>
                <div class="mt-2 border-t border-green-300 pt-2 text-xs text-green-600">Available</div>
              <?php endif; ?>
            </div>
          <?php endforeach; ?>
        </div>
    ...
    <div>
      <?php if ($myAssignment): ?>
        <section class="card border-2 border-yellow-300 bg-yellow-50">
          ...
          <div class="space-y-3">
            <div>
              <div class="text-xs font-medium text-yellow-700">Booth Number</div>
              <div class="text-2xl font-bold text-yellow-900"><?= h($myAssignment['number_blo']) ?></div>
            </div>

            <div class="rounded bg-white p-3">
              <div class="mb-1 text-xs font-medium text-gray-600">Location</div>
            ...
            <div class="rounded bg-white p-3">
              <div class="mb-1 text-xs font-medium text-gray-600">Zone Type</div>
              <div class="text-sm">
            ...
            <?php if (!empty($myAssignment['notes_bas'])): ?>
              <div class="rounded bg-white p-3">
                <div class="mb-1 text-xs font-medium text-gray-600">Notes from Admin</div>
      ...
      <?php else: ?>
        <section class="card">
          ...
          <div class="rounded border border-blue-200 bg-blue-50 p-4 text-center">
            <p class="mb-2 text-sm text-gray-600">No booth assigned yet</p>
            <p class="text-muted text-xs">Booth assignments are made by market administrators based on your product types and market needs.</p>
          </div>
        </section>
      <?php endif; ?>


      <section class="card mt-4 space-y-3 text-xs">
