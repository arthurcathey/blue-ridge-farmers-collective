<section class="card">
  ...
  <p class="text-muted text-sm mb-4">Track your profile visibility, customer engagement, and market performance.</p>
</section>
    ...
    <div class="card-metric">
      ...
      <div class="metric-value">
        <?php if (($metrics['avg_rating'] ?? 0) > 0): ?>
          <span class="text-yellow-500">★</span> <?= number_format($metrics['avg_rating'], 1) ?>
        <?php else: ?>
  ...
  <?php else: ?>
    <div class="grid gap-4 grid-cols-1 sm:grid-cols-2 mb-6">
        ...
        <div class="space-y-2">
          <?php for ($star = 5; $star >= 1; $star--): ?>
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm text-muted w-12"><?= $count ?></span>
            </div>
          <?php endfor; ?>
        </div>
      </div>
      ...
      <div class="bg-gray-50 p-4 rounded border border-gray-200">
        ...
        <div class="space-y-3">
          <div>
            <div class="text-sm text-muted">Approved Reviews</div>
            <div class="text-2xl font-bold text-green-600"><?= ($metrics['approved_reviews'] ?? 0) ?></div>
          </div>
          <div>
            <div class="text-sm text-muted">Pending Approval</div>
            <div class="text-2xl font-bold text-yellow-600"><?= ($metrics['pending_reviews'] ?? 0) ?></div>
          </div>
          <div>
            <div class="text-sm text-muted">Responded</div>
    ...
    <div class="bg-gray-50 p-4 rounded border border-gray-200">
      <h3 class="font-semibold mb-4">Highest Rated Reviews</h3>
          ...
          <?php foreach (array_slice($topReviews, 0, 3) as $review): ?>
            <div class="border-b border-gray-200 pb-3 last:border-0">
              <div class="flex items-center justify-between mb-1">
                ...
                <span class="text-xs text-muted"><?= date('M j', strtotime($review['created_at_vre'])) ?></span>
              </div>
  ...
  <?php else: ?>
    <div class="space-y-3">
      <?php foreach ($marketHistory as $attendance): ?>
        <div class="border border-gray-200 p-4 rounded flex items-center justify-between">
          <div class="flex-1">
            <h3 class="font-semibold"><?= h($attendance['name_mkt']) ?></h3>
            <div class="flex items-center gap-3 flex-wrap mt-1">
              <span class="text-sm text-muted"><?= date('M j, Y', strtotime($attendance['date_mda'])) ?></span>
              ...
              <?php if (!empty($attendance['booth_number_vat'])): ?>
                <span class="text-sm text-muted">Booth <?= h($attendance['booth_number_vat']) ?></span>
              <?php endif; ?>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-3">
        <?php foreach (array_slice($searchVisibility, 0, 10) as $search): ?>
          <div class="flex items-center justify-between border-b border-gray-200 pb-3 last:border-0">
            <div>
              <div class="font-medium text-sm"><?= h($search['search_term']) ?></div>
              <div class="text-xs text-muted">Searched <?= $search['frequency'] ?> time<?= $search['frequency'] != 1 ? 's' : '' ?></div>
            </div>
...
<section class="card mt-6 bg-blue-50 border-l-4 border-blue-500">
