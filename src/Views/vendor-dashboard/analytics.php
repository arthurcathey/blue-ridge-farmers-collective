<section class="card">
  ...
  <p class="text-muted mb-4 text-fluid-sm">Track your profile visibility, customer engagement, and market performance.</p>
</section>
  ...
  <?php else: ?>
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div class="space-y-2">
          <?php for ($star = 5; $star >= 1; $star--): ?>
            ...
            <div class="flex items-center gap-2">
              <span class="w-8 text-fluid-sm font-medium"><?= $star ?>★</span>
              <div class="h-6 flex-1 rounded bg-gray-200">
                <div class="h-6 rounded bg-yellow-400" style="width: <?= $percent ?>%"></div>
              </div>
              <span class="text-muted w-12 text-fluid-sm"><?= $count ?></span>
            </div>
          <?php endfor; ?>
        </div>
      </div>
      ...
      <div class="rounded border border-gray-200 bg-gray-50 p-4">
        ...
        <div class="space-y-3">
          <div>
            <div class="text-muted text-fluid-sm">Approved Reviews</div>
            <div class="text-fluid-2xl font-bold text-green-600"><?= ($metrics['approved_reviews'] ?? 0) ?></div>
          </div>
          <div>
            <div class="text-muted text-fluid-sm">Pending Approval</div>
            <div class="text-fluid-2xl font-bold text-orange-700"><?= ($metrics['pending_reviews'] ?? 0) ?></div>
          </div>
          <div>
            <div class="text-muted text-fluid-sm">Responded</div>
            <div class="text-fluid-2xl font-bold text-brand-primary"><?= ($metrics['responses_count'] ?? 0) ?></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="rounded border border-gray-200 bg-gray-50 p-4">
      ...
      <?php if (empty($topReviews)): ?>
        <p class="text-muted text-fluid-sm">No reviews yet</p>
      <?php else: ?>
        <div class="space-y-3">
          <?php foreach (array_slice($topReviews, 0, 3) as $review): ?>
            <div class="border-b border-gray-200 pb-3 last:border-0">
              <div class="mb-1 flex items-center justify-between">
                ...
                <span class="text-muted text-fluid-xs"><?= date('M j', strtotime($review['created_at_vre'])) ?></span>
              </div>
              <p class="line-clamp-2 text-fluid-sm text-gray-700">
        ...
        <div class="flex items-center justify-between rounded border border-gray-200 p-4">
          <div class="flex-1">
            ...
            <div class="mt-1 flex flex-wrap items-center gap-3">
              <span class="text-muted text-fluid-sm"><?= date('M j, Y', strtotime($attendance['date_mda'])) ?></span>
              <span class="inline-flex items-center px-2 py-1 rounded text-fluid-xs text-white <?php
              ...
              <?php if (!empty($attendance['booth_number_vat'])): ?>
                <span class="text-muted text-fluid-sm">Booth <?= h($attendance['booth_number_vat']) ?></span>
              <?php endif; ?>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-3">
        <?php foreach (array_slice($searchVisibility, 0, 10) as $search): ?>
          <div class="flex items-center justify-between border-b border-gray-200 pb-3 last:border-0">
            <div>
              <div class="text-fluid-sm font-medium"><?= h($search['search_term']) ?></div>
              <div class="text-muted text-fluid-xs">Searched <?= $search['frequency'] ?> time<?= $search['frequency'] != 1 ? 's' : '' ?></div>
            </div>
            <div class="text-right">
              <div class="font-semibold text-brand-primary"><?= $search['last_30_days'] ?></div>
              <div class="text-muted text-fluid-xs">Last 30 days</div>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
...
<section class="card mt-6 border-l-4 border-blue-500 bg-blue-50">
  ...
  <ul class="space-y-2 text-fluid-sm text-gray-700">
