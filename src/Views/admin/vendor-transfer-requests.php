  ...
  <div class="max-w-6xl">
    ...
    <div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-4">
      ...
      <div class="card">
        ...
        <p class="text-2xl font-bold text-green-600"><?= (int) ($stats['approved'] ?? 0) ?></p>
      </div>
      <div class="card">
        <p class="text-muted mb-1 text-sm">Rejected</p>
        <p class="text-2xl font-bold text-red-600"><?= (int) ($stats['rejected'] ?? 0) ?></p>
      </div>
      <div class="card">
        <p class="text-muted mb-1 text-sm">Total Requests</p>
        <p class="text-2xl font-bold text-gray-600"><?= (int) ($stats['total'] ?? 0) ?></p>
      </div>
    </div>
      ...
      <div class="space-y-4">
        <?php foreach ($requests as $request): ?>
          ...
          <div class="rounded-lg border border-gray-200 p-4 md:p-6">
            <div class="mb-4 flex flex-col items-start justify-between gap-4 md:flex-row md:items-start">
              <div class="flex-1">
                <div class="mb-2 flex items-center gap-3">
                  ...
                  <span class="text-muted text-sm">(<?= h($request['username_acc']) ?>)</span>
                </div>
            ...
            <div class="mb-4 grid grid-cols-1 gap-4 text-sm md:grid-cols-3">
              <div>
                <p class="text-muted mb-1">From Market</p>
                <p class="font-semibold"><?= h($request['from_market']) ?></p>
                <p class="text-muted text-xs"><?= h($request['from_city'] . ', ' . $request['from_state']) ?></p>
              </div>
              <div>
                <p class="text-muted mb-1">To Market</p>
                <p class="font-semibold"><?= h($request['to_market']) ?></p>
                <p class="text-muted text-xs"><?= h($request['to_city'] . ', ' . $request['to_state']) ?></p>
              </div>
              <div>
                <p class="text-muted mb-1">Vendor</p>
                <p class="font-semibold"><?= h($request['farm_name_ven']) ?></p>
                <p class="text-muted text-xs"><?= h($request['city_ven'] . ', ' . $request['state_ven']) ?></p>
              </div>
            </div>

            <?php if (!empty($request['notes_vtr'])): ?>
              <div class="mb-4 rounded border-l-4 border-blue-300 bg-blue-50 p-3">
                <p class="mb-1 text-sm font-semibold text-blue-900">Vendor's Reason</p>
                <p class="text-sm text-blue-800"><?= nl2br(h($request['notes_vtr'])) ?></p>
              </div>
            <?php endif; ?>

            <?php if (!empty($request['admin_notes_vtr'])): ?>
              <div class="mb-4 rounded border-l-4 border-gray-300 bg-gray-50 p-3">
                <p class="mb-1 text-sm font-semibold text-gray-900">Admin Notes</p>
                <p class="text-sm text-gray-700"><?= nl2br(h($request['admin_notes_vtr'])) ?></p>
              </div>
            <?php endif; ?>

            <?php if (!empty($request['processed_at_vtr'])): ?>
              <div class="text-muted mb-4 border-t border-gray-200 pt-3 text-xs">
                <p>
...
<div id="rejectModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-4">
  <div class="w-full max-w-md rounded-lg bg-white p-6">
    <h2 class="mb-4 text-lg font-semibold">Reject Transfer Request</h2>
    ...
    <form onsubmit="submitReject(event)" class="space-y-4">
      ...
      <div>
        <label for="adminNotes" class="mb-2 block text-sm font-medium text-gray-900">
        ...
        <p class="text-muted mt-1 text-xs">Max 1000 characters</p>
      </div>
