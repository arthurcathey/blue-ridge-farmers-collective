...
class ProductController extends BaseController
{
  ...
  private function storeProductPhoto(?array $file): array
  {
    ...
    if (($file['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK) {
      return ['path' => null, 'error' => 'Photo upload failed.'];
    }

    $maxBytes = 2 * 1024 * 1024;
    if (($file['size'] ?? 0) > $maxBytes) {
      return ['path' => null, 'error' => 'Photo must be 2MB or smaller.'];
    }
    ...
    if (!isset($allowed[$mime])) {
      return ['path' => null, 'error' => 'Photo must be a JPG, PNG, or WebP image.'];
    }

    $uploadDir = $this->basePath . '/public/uploads/products';
    if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
      return ['path' => null, 'error' => 'Unable to save photo.'];
    }
  ...
  public function update(): string
  {
    ...
    if (!$errors) {
      $photoResult = $this->storeProductPhoto($_FILES['photo'] ?? null);
  ...
  public function index(): string
  {
    ...
    try {
      $stmt = $db->query('SELECT id_pct, name_pct FROM product_category_pct ORDER BY display_order_pct ASC, name_pct ASC');
      $categories = $stmt ? $stmt->fetchAll() : [];
    } catch (\Throwable $e) {
      $categories = [];
    }
    ...
    try {
      $stmt = $db->query('SELECT id_ven, farm_name_ven FROM vendor_ven WHERE application_status_ven = "approved" ORDER BY farm_name_ven ASC');
    ...
    try {
      $stmt = $db->query('SELECT id_mkt, name_mkt FROM market_mkt WHERE is_active_mkt = 1 ORDER BY name_mkt ASC');
